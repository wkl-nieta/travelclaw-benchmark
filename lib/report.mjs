...
// Support both new { meta, rounds } format and legacy plain array
const meta   = raw.meta ?? null;
const data   = Array.isArray(raw) ? raw : raw.rounds;
const cs     = meta?.cold_start ?? null;
...
const coldBlock = cs ? (() => {
  const phases = [
    { key: 'clone',   label: 'Clone',   ms: cs.clone_ms,         cls: 'clone' },
    { key: 'soul',    label: 'Soul',    ms: cs.soul_ms,          cls: 'soul'  },
    { key: 'suggest', label: 'Suggest', ms: cs.first_suggest_ms, cls: 'suggest' },
    { key: 'gen',     label: '1st Gen', ms: cs.first_gen_ms,     cls: 'gen'   },
  ];
  const maxMs = Math.max(...phases.map(p => p.ms), 1);
  const bars = phases.map(p => {
    const pct = Math.max((p.ms / maxMs) * 100, 1).toFixed(1);
    const label = p.ms > 400 ? fmtMs(p.ms) : '';
    return `<div class="wf-row">
      <div class="wf-label">${p.label}</div>
      <div class="wf-bar-wrap"><div class="wf-bar ${p.cls}" style="width:${pct}%">${label}</div></div>
      <div class="wf-ms">${fmtMs(p.ms)}</div>
    </div>`;
  }).join('');
  const freshLabel = cs.fresh_clone ? '<span style="color:#2f9e44">fresh clone</span>' : '<span style="color:#888">cached clone</span>';
  return `<div class="cold">
  <div class="cold-title">Cold Start (${freshLabel})</div>
  <div class="waterfall">${bars}</div>
  <div class="cold-total">Total: <span>${fmtMs(cs.total_ms)}</span></div>
  <div class="cold-note">Time from repo checkout to first image returned · gen time varies by server load</div>
</div>`;
})() : '';
...
.cold{background:#fff;border:1px solid #e8eaed;border-radius:10px;padding:1.2rem 1.4rem;margin-bottom:2rem;box-shadow:0 1px 3px rgba(0,0,0,.06)}
.cold-title{font-size:.8rem;font-weight:600;color:#555;margin-bottom:.8rem;text-transform:uppercase;letter-spacing:.05em}
.waterfall{display:flex;flex-direction:column;gap:.45rem}
.wf-row{display:flex;align-items:center;gap:.7rem;font-size:.8rem}
.wf-label{width:80px;text-align:right;color:#666;flex-shrink:0}
.wf-bar-wrap{flex:1;background:#f0f2f5;border-radius:4px;height:18px;position:relative}
.wf-bar{height:100%;border-radius:4px;display:flex;align-items:center;padding:0 6px;font-size:.7rem;font-weight:600;color:#fff;white-space:nowrap;min-width:2px}
.wf-bar.clone{background:#7950f2}.wf-bar.soul{background:#1c7ed6}.wf-bar.suggest{background:#2f9e44}.wf-bar.gen{background:#e67700}
.wf-ms{width:62px;color:#444;font-size:.78rem;flex-shrink:0}
.cold-total{margin-top:.9rem;font-size:1rem;font-weight:700;color:#1a1a1a}
.cold-total span{color:#3b5bdb}
.cold-note{font-size:.73rem;color:#888;margin-top:.3rem}
